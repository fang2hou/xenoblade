# Xenoblade Architecture Documentation

## Architecture Decision Records (ADRs)

ADRs capture significant architectural decisions, their context, and consequences.

| # | Title | Status |
|---|-------|--------|
| [001](adr/001-migrate-to-discordjs.md) | Migrate from Chat SDK to discord.js | Accepted |
| [002](adr/002-hybrid-deployment.md) | Hybrid Deployment Topology | Accepted |
| [003](adr/003-single-shot-generation.md) | Single-Shot Generation Protocol | Accepted |
| [004](adr/004-two-tier-model.md) | Two-Tier Model Architecture | Accepted |
| [005](adr/005-dm-control-plane-memory.md) | DM Control Plane and Per-User Memory | Accepted |
| [006](adr/006-smart-url-reader.md) | Smart URL Reader Pipeline | Accepted |
| [007](adr/007-multi-backend-search.md) | Multi-Backend Search | Accepted |
| [008](adr/008-mcp-integration.md) | MCP Integration Scope | Accepted |
| [009](adr/009-fresh-database.md) | Fresh D1 Database | Accepted |
| [010](adr/010-cicd-docker-ssh.md) | CI/CD with Docker Registry and SSH | Accepted |

## ADR Format

Each ADR follows this structure:

- **Status** — Accepted, Superseded, or Deprecated
- **Context** — Why this decision was needed
- **Decision** — What was decided
- **Consequences** — Positive, negative, and neutral implications
